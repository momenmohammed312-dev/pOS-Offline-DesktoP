class ArabicReshaperConfig {
  final String language;
  final bool deleteHarakat;
  final bool shiftHarakatPosition;
  final bool deleteTatweel;
  final bool supportZwj;
  final bool useUnshapedInsteadOfIsolated;
  final bool supportLigatures;

  /// Map of specific ligature names to enabled/disabled status.
  final Map<String, bool> _ligatureConfig;

  const ArabicReshaperConfig({
    this.language = 'Arabic',
    this.deleteHarakat = true,
    this.shiftHarakatPosition = false,
    this.deleteTatweel = false,
    this.supportZwj = true,
    this.useUnshapedInsteadOfIsolated = false,
    this.supportLigatures = true,
    Map<String, bool> ligatureOverrides = const {},
  }) : _ligatureConfig = ligatureOverrides;

  /// Checks if a specific ligature is enabled.
  /// 1. If [supportLigatures] is false, returns false (unless specific logic overrides, but standard behavior implies master switch).
  /// 2. Checks user overrides passed in constructor.
  /// 3. Checks the default configuration map.
  /// 4. Defaults to false.
  bool isLigatureEnabled(String name) {
    if (!supportLigatures) return false;

    // 1. Check User Overrides
    if (_ligatureConfig.containsKey(name)) {
      return _ligatureConfig[name]!;
    }

    // 2. Check Default Configuration
    if (_defaultLigatures.containsKey(name)) {
      return _defaultLigatures[name]!;
    }
  ...
  // -------------------------------------------------------------------------
  // DEFAULT CONFIGURATION DATA (Ported from reshaper_config.py)
  // -------------------------------------------------------------------------
  static const Map<String, bool> _defaultLigatures = {
